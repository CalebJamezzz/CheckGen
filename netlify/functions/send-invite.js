/* send-invite.js — Netlify function for workspace invite emails
 * Called from team.html after inserting into workspace_invites table
...
exports.handler = async (event) => {
  ...
  if (!RESEND_API_KEY) {
    // No Resend key — return the invite URL so admin can share manually
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ok: false, reason: 'no_smtp', message: 'Email sending not configured. Share the invite link manually.' })
    };
  }
  ...
  const { to, inviterName, workspaceName, inviteUrl } = body;
  if (!to || !inviteUrl) {
    return { statusCode: 400, body: 'Missing required fields: to, inviteUrl' };
  }

  // Build email HTML inline (matches site design)
