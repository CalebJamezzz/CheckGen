...
function renderNav(user, profile) {
  ...
  el.innerHTML = `
    ...
    <nav class="nav-links">
      ...
      <a href="${isAnon ? '/signup.html?reason=history' : '/history.html'}" class="nav-link ${p.includes('history')?'active':''}">
        ...
        History${isAnon?'<span class="nav-lock">🔒</span>':''}
      </a>
      <a href="${isAnon ? '/signup.html?reason=team' : '/team.html'}" class="nav-link ${p.includes('team')?'active':''}">
        ...
        Team${isAnon?'<span class="nav-lock">🔒</span>':''}
      </a>
    </nav>
    <div class="nav-bottom">
      <a href="${isAnon ? '/signup.html?reason=account' : '/account.html'}" class="nav-user ${p.includes('account')?'active':''}">
        <div class="nav-avatar">${isAnon ? '?' : name.charAt(0).toUpperCase()}</div>
        <div class="nav-user-info">
          <div class="nav-user-name">${isAnon ? 'Guest' : escNav(name)}</div>
          <div class="nav-user-tier">${isAnon ? 'No account' : 'Free'}</div>
        </div>
      </a>
      ${isAnon
