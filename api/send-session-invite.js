/* send-session-invite.js — Vercel function, emails invited users when a shared session is created
...
export default async function handler(req, res) {
  ...
  const { to, inviterName, sessionName, ticketId, shareCode, sessionUrl } = req.body || {};
  if (!to || !shareCode) return res.status(400).send('Missing required fields');
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#080B12;">
  ...
  if (!RESEND_API_KEY) {
    return res.status(200).json({ ok: false, reason: 'no_smtp', joinUrl });
  }
